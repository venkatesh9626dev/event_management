...
function viewMore (eventId,eventDate,eventTime) {
  ...
  async function loadParticipatorForm(categoryList, eventDetails) {
    ...
    let categoryCount = await categoryCountCheck(fragment,categoryList,eventDetails);
    ...
    try{
      ...
      for(let element of checkboxes){
        if(element.checked){
          ...
          let eventRef = ref(db,`events/${eventDetails.eventId}/subCategory/${element.id}/participantDetails/${userId}`)
          
          let data = {
            userName: dataObj.userName,
            mailId : dataObj.mailAddress,
